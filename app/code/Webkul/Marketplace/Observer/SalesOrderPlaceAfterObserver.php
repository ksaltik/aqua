...
class SalesOrderPlaceAfterObserver implements ObserverInterface
{
    ...
    {//echo "here";exit;
        ...
        //$logger->info(print_r($collectionsave, true));
        $lastOrderId = $order->getId();
        /*
        * Get Global Commission Rate for Admin
        */
        ...
        */
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            list($sellerId, $originalProductId,$vendorPurchasePrice)=$this->getSellerIdPerProduct($item);


            // echo $vendorPurchasePrice;exit;
            ...
            if($originalProductId==0) {
                ...
                /*Vendor commission on admin product*/
                $collectionsave->setVendorTotalCommission(0);
                $collectionsave->setActualAdminAmount(0);
                $collectionsave->setVendorCommissionRate(0);
                $collectionsave->setVendorTotalAmount(0);
                //$collectionsave->setTotalCommission($originalProductId);


                /*Close*/
            }else{
                /*Vendor commission on admin product*/
                $vendorPurchasePriceWithQty=$qty*$vendorPurchasePrice;
                $actualSeller=$actparterprocost-$vendorPurchasePriceWithQty;
                $collectionsave->setTotalCommission($vendorPurchasePriceWithQty);
                $collectionsave->setActualSellerAmount($actualSeller);
                $collectionsave->setCommissionRate(0);
                /*Vendor benefit amount*/
                $benefit=$price-$vendorPurchasePrice;
                $benefit=$qty*$benefit;
                if (isset($itemData['base_discount_amount']) && (int) $itemData['base_discount_amount']) {
                    $baseDiscountAmount = $itemData['base_discount_amount'];
                    $benefit = $benefit - $baseDiscountAmount;
                }
                $collectionsave->setVendorBenefitAmount($benefit);
                /*Close*/
                /*$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/test.log');
                $logger = new \Zend\Log\Logger();
                $logger->addWriter($writer);
                $logger->info("Test-".$originalProductId.'-'.$vendorPurchasePrice.'-'.$lastOrderId);*/
            }
            $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/test.log');
            $logger = new \Zend\Log\Logger();
            $logger->addWriter($writer);
            //$logger->info("Test-".$collectionsave->getData());
    ...
    public function getVendorCommission($sellerId, $totalamount, $item, $advanceCommissionRule)
    {
        /*
        * Get Global Commission Rate for Admin
        */
        ...
        if ($collection1->getSize() != 0) {
            foreach ($collection1 as $rowdatasale) {
                $commission = ($totalamount * $rowdatasale->getVendorCommissionRate()) / 100;
            }
        } else {
            $commission = ($totalamount * $percent) / 100;
        }
    ...
    public function getSellerIdPerProduct($item)
    {
        ...
        if ($mpassignproductId) {
            ...
            $originalProductId = $mpassignModel->getOriginalProductId();
            $vendorPurchasePrice = $mpassignModel->getVendorPurchasePrice();
        ...
        if ($sellerId == '') {
            ...
            foreach ($collectionProduct as $value) {
                ...
                $originalProductId = $value->getOriginalProductId();
                $vendorPurchasePrice = $value->getVendorPurchasePrice();
            }
        }
        if ($sellerId == '') {
            $sellerId = 0;
        ...
        if ($vendorPurchasePrice == '') {
            $vendorPurchasePrice = 0;
        }
        $data[]=$sellerId;
        $data[]=$originalProductId;
        $data[]=$vendorPurchasePrice;
