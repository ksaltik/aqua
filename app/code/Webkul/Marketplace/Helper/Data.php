...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const XML_PATH_MODULE_ENABLE = 'marketplace/general_settings/enable_module';
    ...
    public function getConfigVendorCommissionRate()
    {
        return $this->scopeConfig->getValue(
            'marketplace/general_settings/merchantpercent',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }

    public function getVendorCommissionRate($sellerID){
        $vendorCommission=0;
        $collection = $this->_objectManager->create(
        ...
        if (count($collection)) {
            foreach ($collection as $value) {
                $vendorCommission=$value->getVendorCommissionRate();
            }
        } else {
            $vendorCommission = $this->_objectManager->create(
     ...
     * @return \Webkul\Marketplace\Model\ResourceModel\Product\Collection
     */
    public function getMarketProductData()
    {
        $model = $this->_objectManager->create(
            'Webkul\Marketplace\Model\Product'
        )->getCollection();
