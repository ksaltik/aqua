...
class Facebookwallpost extends \Magento\Framework\View\Element\Template
{
    ...
    private function getvalidAccessToken()
    {
        ...
        $curlInitiateFn = curl_init();

        /*
         * intiate curl request to graph api to get access token 
         */
        curl_setopt(
            $curlInitiateFn,
            CURLOPT_URL,
        ...
        curl_setopt($curlInitiateFn, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));

        $longaccesstoken = curl_exec($curlInitiateFn);

        curl_close($curlInitiateFn);

        $response = json_decode($longaccesstoken);
        if ($response != null) {
            /*
