	...
	$.fn.webkulfbwall = function(options) {
		...
		return meta.each(function() {
			...
			var initBase = function(data){
				...
				var fields = ["actions","caption","created_time","comments","description","from","full_picture","icon,id","likes.limit(1000)","link","message","message_tags","name","object_id","picture","properties","source","status_type","story","story_tags","subscribed","to","type","privacy","application","updated_time"]
			...
			var initWall = function(data){	
				...
				for(var k=0;k<max;k++){
					// console.log('okke '+data[k].comments);
					...
					if(typeof data[k].actions!='undefined'){
						...
						if(exists(data[k].comments) && exists(data[k].comments.data) && (o.showComments==true||o.showComments=='true')){
							...
							for(var c=0;c<data[k].comments.data.length-1;c++){
								...
								console.log('keeee '+data[k].comments.data[c].from);
