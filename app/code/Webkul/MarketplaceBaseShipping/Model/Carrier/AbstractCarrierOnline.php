...
abstract class AbstractCarrierOnline extends CoreAbstractCarrierOnline
{
    ...
    {
        $routeName = $this->requestInterface->getRouteName();
        if ($routeName == 'wkpickup' && $this->_scopeConfig->getValue('carriers/wkpickup/active')) {
            return false;
        } elseif ($this->_scopeConfig->getValue('carriers/wkpickup/active')) {
            return true;
        }
        return false;
    }
    ...
    */
    ...
    protected function _getSellerId($mpassignproductId, $proid)
    {
        ...
        if ($mpassignproductId) {
            $this->assignItemsFactory = ObjectManager::getInstance()->create(\Webkul\MpAssignProduct\Model\Items::class);
    ...
    {
        return $this->_determineShippingDay($this->getConfigData('shipment_days'), date(AbstractDhl::REQUEST_DATE_FORMAT));
    }
    ...
    protected function getCountryParams($countryCode)
    {
        if (empty($this->_countryParams)) {
            ...
            $readFactory = ObjectManager::getInstance()->create(\Magento\Framework\Filesystem\Directory\ReadFactory::class);
