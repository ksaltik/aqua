...
class Massreceive extends Action
{
    /**
    ...
    public function execute()
    {
        ...
        if (!is_array($pickingIds)) {
            ...
            try {
                foreach ($pickingIds as $picking_id) {
                    $picking = $this->_picking->create()->load($picking_id);
                    if($picking->getStatus() == 2)
                        continue;
                    $this->_helper->changePickingStatus($picking_id, 2);
                    ...
                    if ($purchase_id) {
                        ...
                        foreach ($items as $item) {
                            ...
                            $net_received = $receive_qty + ($quantity - $receive_qty);
