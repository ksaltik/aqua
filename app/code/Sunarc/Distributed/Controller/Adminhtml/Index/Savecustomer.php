...
namespace Sunarc\Distributed\Controller\Adminhtml\Index;
...
class Savecustomer extends \Magento\Customer\Controller\Adminhtml\Index\Save
{	
        ...
        if ($originalRequestData) {
            try {
                ...
                if ($customerId) {
                    ...
                    if( isset($originalRequestData['customer']['customergroup']) && $originalRequestData['customer']['customergroup'] != null ){
                        $this->sendmailonchnagecustomergroup($originalRequestData);
                    }
    ...
    private function sendmailonchnagecustomergroup($originalRequestData){

        $newGroupId = $originalRequestData['customer']['group_id'];

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $customerRepository = $objectManager->get('Magento\Customer\Api\CustomerRepositoryInterface');
        $scopeConfig = $objectManager->get('Magento\Framework\App\Config\ScopeConfigInterface');
        $custmr = $customerRepository->getById($originalRequestData['customer']['entity_id']);
        $oldGroupId = $custmr->getGroupId();
        
        if( $newGroupId != $oldGroupId ){
            $groupRepository  = $objectManager->create('\Magento\Customer\Api\GroupRepositoryInterface');
            $newGroupName = $groupRepository->getById($newGroupId)->getCode();
            $oldGroupName = $groupRepository->getById($oldGroupId)->getCode();
            ...
            $templateVars = array(
                                ...
                                'email' => $custmr->getEmail(),
                                'name' => $custmr->getFirstname().' '.$custmr->getLastname(),
                                'newGroup' => $newGroupName,
                                'oldGroup' => $oldGroupName,
                            );
            $from = array('email' => $storeEmail, 'name' => $storeName);
            $to = array($custmr->getEmail());
