...
class salesQuoteItemSetSplitorderAttribute implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $optionsValue=[];
        $splitAttributeId='';
        $splitAttributeId=$this->_objectManager->create('Sunarc\AdvPermission\Helper\Data')->getSplitAttrId();
        if ($splitAttributeId !='') {
        //get attribute code by which order will split
            $splitAttributeCode =$this->_objectManager->create('Sunarc\AdvPermission\Helper\Data')->getSplitAttrCode($splitAttributeId);
          //get productIds on the basis of attribute options
            list($splitAttribute,$attrOptionArray) = $this->_objectManager->create('Sunarc\AdvPermission\Helper\Data')->getsplitorderCollection();
